...
function ttf2woff2Transform() {
    ...
    return function ttf2woff2TransformCb(err, buf, cb) {
        ...
        try {
            ...
            return cb(new PluginError(PLUGIN_NAME, err2.message, {
                showStack: true,
            }));
        }
    };
...
function ttf2woff2Gulp(options) {
    ...
    stream._transform = function ttf2woff2GulpTransform(file, _, done) {
        let cntStream;
        let newFile;
        ...
        if (options.clone) {
            if (file.isBuffer()) {
                ...
                newFile.contents = file.contents.pipe(new PassThrough());
                stream.push(newFile);
            }
        }
        ...
        if (file.isBuffer()) {
            try {
                ...
                stream.emit('error', new PluginError(PLUGIN_NAME, err.message, {
                    showStack: true,
                }));
            }
            // Streams
            ...
            file.contents = file.contents.pipe(new BufferStream(ttf2woff2Transform()));
        }
        stream.push(file);
        done();
    };
